...
export function BulkGenerator({
  ...
  const [results, setResults] = useState<
    { email: string; result: boolean }[] | null
  >(null);
  ...
  function downloadResultsCsv() {
    ...
    const rows = results.map(
      (r) => `${r.email},${r.result ? "Success" : "Failed"}`
    );
  ...
  return (
    <div className="mt-6 space-y-3 rounded-lg border border-zinc-200 bg-white/70 p-4 text-xs shadow-sm dark:border-zinc-700 dark:bg-zinc-800/70">
      ...
      <p className="text-[11px] text-zinc-500">
        1. Create text layers (field_1..n). 2. Download template. 3. Fill CSV.
        4. Upload to generate ZIP. 5. Save Certificates to send emails.
      </p>
      <input
        type="file"
        accept=".csv"
      ...
      <button
        ...
        onClick={onGenerate}
        className={`w-full rounded-md px-3 py-2 text-[11px] font-medium text-white shadow ${
          generating || csvRows.length === 0
      ...
      <button
        type="button"
        disabled={!onSaveCertificates || csvRows.length === 0}
        onClick={onSaveCertificates}
        className={`w-full rounded-md px-3 py-2 text-[11px]
          font-medium text-white shadow ${
            !onSaveCertificates || csvRows.length === 0
              ? "bg-zinc-400 dark:bg-zinc-600 cursor-not-allowed"
      ...
      <p className="text-[10px] text-zinc-500">
        Provide a verified From email, subject and message body, then send
        certificates to emails in the uploaded CSV.
      </p>
      <input
        type="email"
      ...
      <button
        ...
        onClick={handleSend}
        className={`w-full rounded-md px-3 py-2 text-[11px] font-medium text-white shadow ${
          sending || csvRows.length === 0 || !onSendEmails
            ? "bg-zinc-400 dark:bg-zinc-600 cursor-not-allowed"
      ...
      {sendMsg && (
        <p className="text-[11px] mt-1 font-medium text-zinc-600 dark:text-zinc-300">
          {sendMsg}
        </p>
      )}
